  ...
  routes: function( app ){
    ...
    app.put( '/documents/:id/comments/:commentId', auth.check, getDocument, function( req, res ){
      if( req.doc ){
        ...
        if( comment._user.toString() === res.locals.currentUser._id.toString() || res.locals.currentUser.groups.indexOf('manager') >= 0 ){
...
function getDocument( req, res, next ){
  ioco.db.firstAnyWithUser( res.locals.currentUser, { 
    _id: ioco.db.Types.ObjectId( req.params.id ) }, 
    function( err, doc ){
      req.doc = doc;
      next();
  });
