...
var filesPlugin = {
  ...
  routes: function( app ){
          ...
          if( file )
            ...
            res.format({
              ...
              html: function(){
                ...
                var f = path.join( ioco.config.datastore.absolutePath, 'files', file.getFilePath() );
                if( req.query.size && req.query.size === '50' ){
                  var thumb = path.basename(f).split('.')[0]+'_thumb_'+req.query.size+'x'+req.query.size+path.extname(f)
                  if( !fs.existsSync( thumb ) )
                    filetools.thumb( f, {w: parseInt(req.query.size), h: parseInt(req.query.size)}, function( err, thumb ){
                      res.sendfile( thumb ); 
                    });
                  else
                    res.sendfile( thumb );
    ...
    app.delete( '/files/:id', auth.check, files.getById, function( req, res ){
      if( req.file && ( res.locals.currentUser.isEditor() || req.file.canDelete() ) )
