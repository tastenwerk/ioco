...
function crop( filepath, newfilepath, coords, callback ){
  ...
  imProc.size( function( err, size ){
    if( err )
      console.log(err);
    imProc.crop(coords.w,coords.h,coords.x,coords.y).write( newfilepath, function (err) {
      if( err ) console.log(err);
      console.log('cropped to dimensions', coords.w, coords.h, ' with coords ', coords.x, coords.y, filepath, newfilepath);
      callback( err )
    });
  });
}
