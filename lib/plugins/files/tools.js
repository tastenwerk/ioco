...
function resize( filepath, dimensions, callback ){
  ...
  var resizeOpts = { src: origFilepath, dst: filepath, width: parseInt(dimensions.split('x')[0]), height: parseInt(dimensions.split('x')[1]) };
  easyimg.resize( resizeOpts, function( err, image ){
    console.log('resized to dimensions', dimensions);
    util.pump( fs.createReadStream( origFilepath ), fs.createWriteStream( filepath ), function( err ){
