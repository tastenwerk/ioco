...
function crop( filepath, dimensions, callback ){
  ...
  var imProc = gm( origFilepath );
  if( dimensions.split('x').length > 1 )
    imProc.crop( parseInt(dimensions.split('x')[0]), parseInt(dimensions.split('x')[1]) );
  else
    imProc.crop( parseInt(dimensions), parseInt(dimensions) );

  imProc.write(writeStream, function (err) {
    if( err ) console.log(err);
    console.log('cropped to dimensions', dimensions);
    fs.unlinkSync( origFilepath );
    callback( err )
  });
}
