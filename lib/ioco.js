...
var ioco = {
  ...
  inject: function loadPlugins( _express, app, io ){
    ...
    app.use(function(err, req, res, next){
      ...
      ioco.log.error(err.stack);
      if (req.xhr) {
        res.set
        res.status(500).render( ioco.view.lookup('errors/500.ejs'), { error: err } );
      } else {
        res.status(500).render(ioco.view.lookup('errors/500.jade'), { error: err } );
      }
    });
  ...
  filetools: function(){ return require( __dirname+'/plugins/files/tools' ); }
